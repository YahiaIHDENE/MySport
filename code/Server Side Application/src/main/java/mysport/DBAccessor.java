...
public class DBAccessor{
    ...
    public String mesReservation(@PathParam("id_user") String id_user) {
        ...
        ArrayList<Annonce> mesReserve = new ArrayList<Annonce>();
        ResultSet rs =null;
        String rsaa = null;
        String SELECT = "Select mysport.t_reserve.id_user, mysport.t_reserve.id_reserve, mysport.t_reserve.heure_debut, mysport.t_reserve.heure_fin," +
                                " mysport.t_reserve.jour_reserve, mysport.t_reserve.nombre_reserve, mysport.t_annonce.id_annonce," +
                                " mysport.t_terrain.* from mysport.t_reserve inner join mysport.t_annonce on mysport.t_annonce.id_annonce =" +
                                " mysport.t_reserve.id_annonce inner join mysport.t_terrain on mysport.t_terrain.id_terrain =" +
                                "mysport.t_annonce.id_terrain where mysport.t_reserve.id_user ="+idUserReservation;
        try {
            ps = conn.prepareStatement(SELECT);
            rs = ps.executeQuery();
            ...
            while (rs.next()){
                ...
                a.setTypeItem(typeItem);
                a.setIdAnnonce(rs.getInt("id_reserve"));
                a.setId_userPublier(rs.getInt("id_user"));
                a.setDateDisponible(rs.getString("jour_reserve"));
                a.setHeureDebut(rs.getString("heure_debut"));
                a.setHeureFin(rs.getString("heure_fin"));
                a.setNombreDePlaceRestant(rs.getInt("nombre_reserve"));
                a.setTerrain(terr);
                mesReserve.add(a);
            }
            String json= gson.toJson(mesReserve);
            return (json );
        } catch (Exception e){
            e.printStackTrace();
    ...
    public String editReservation(@PathParam("reservation") String reservation) {
        ...
        String SELECT2 = "SELECT place_restant FROM mysport.t_annonce WHERE id_annonce =(SELECT id_annonce FROM mysport.t_reserve WHERE id_reserve ="+update_reservation.getIdAnnonce()+")" ;
        ...
        try {
            ...
            try {
                ...
                if (rs1.next()) {
                    ...
                    System.out.println(nvx+"="+placeRestant+"+"+ ancienreserve+"-"+update_reservation.getNombreDePlace());
                    if(nvx<0)
                ...
                try {
                    ps = conn.prepareStatement(UPDATE);
                    ps.execute();
                    String Update = "UPDATE  mysport.t_annonce SET place_restant=" +nvx+ " WHERE id_annonce =(SELECT id_annonce FROM mysport.t_reserve WHERE id_reserve ="+update_reservation.getIdReservation()+")" ;
                    try {
                        ps = conn.prepareStatement(Update);
                        ps.execute();
